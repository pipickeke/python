...
class Solution:
    ...
    def profitableSchemes_2(self, n: int, minProfit: int,
        ...
        for i in range(1,m+1):
            ...
            for j in range(n+1):
                for k in range(minProfit+1):
                    f[i][j][k] = f[i-1][j][k]
                    if j >= g:
                        f[i][j][k] += f[i-1][j-g][max(k-p,0)]
                        f[i][j][k] %= mod

        return f[m][n][minProfit]


    def profitableSchemes_3(self, n: int, minProfit: int,
                            group: List[int], profit: List[int]) -> int:
